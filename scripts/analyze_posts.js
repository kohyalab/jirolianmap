...
1. 時間の解釈と日本時間（JST）の厳守:
   ...
   - 「只今並びの方で終了」「昼の営業を終了」などの早仕舞い告知で、投稿時刻を営業終了時刻として採用する場合は、必ず上記「投稿日時（日本時間 JST）」の時刻（例: ${jstHours}:${jstMinutes}）を基準としてください。UTC（協定世界時・9時間前）と誤認しては絶対にいけません（例: 投稿が13:36の場合、終了時刻は13:36（13.6）等であり、4:36ではありません）。
2. 推測での変更提案の厳禁（具体的な時刻記載がない場合は変更提案しない）:
   - 「かなり早めの閉店」「早めの閉店になりそう」「材料なくなり次第終了」「仕込みの関係で早仕舞い」「売り切れ次第終了」など、具体的な営業終了時刻が記載されていない抽象的な表現の場合、営業終了時刻を勝手に推測（例: 18:00〜18:00等）して変更提案を作らないでください。具体的な時刻が指定されていない場合は必ず hasScheduleChange: false としてください。
3. 投稿時刻以前の過去の営業シフト・および言及のないシフトの改変厳禁:
   - 投稿時刻より前に既に終了している過去の営業シフト（例: 14:50投稿における昼の部）について、投稿文に明示的な変更の言及がない限り、絶対に修正を加えないでください。
   - 投稿文で言及されていないシフト（例: 夜の部の案内のみで昼の部に言及がない場合）は、店舗の通常登録営業時間の数値をそのまま維持してください（例: 札幌店の昼が10:30〜14:00の場合、夜の案内投稿で昼を勝手に11:00〜14:00に変更しては絶対にいけません）。
4. 通常営業スケジュールと同一内容の投稿は変更なし（hasScheduleChange: false）:
   - 投稿文に記載された営業時間が、対象曜日の通常登録営業時間と同一である場合（例: 月曜夜が通常17:00〜20:00の店舗が「21日(月)夜の部17:00〜20:00です」と投稿した場合）、営業時間の変更ではないため、必ず hasScheduleChange: false としてください。
   - 「本日も通常通り営業します」「仕込み中です」「限定トッピングあります」等の日常投稿も hasScheduleChange: false としてください。
5. 日付・期間表現の解釈:
   - 「本日」「今日」＝ ${todayStr}
   - 「明日」＝ 翌日、「明後日」＝ 翌々日
   - 「○日(○)」＝ 今月または翌月の該当月日を西暦YYYY-MM-DD形式に変換。
   - カレンダー画像がある場合、〇印（営業）、✕印や斜線（休業）、手書きの注釈を正確に読み取ること。
6. 昼の部・夜の部、および片側休業の厳格な解釈:
...
7. リアルタイム営業終了アナウンス（早仕舞い）の終了時刻反映:
